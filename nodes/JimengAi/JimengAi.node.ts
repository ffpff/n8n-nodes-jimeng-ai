  ...
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      },
      {
        ...
        description:
          "控制图片生成的随机性，-1 表示随机生成。使用相同种子和提示词可以生成相似图片",
      },
      // 图像尺寸
      {
        displayName: "尺寸配置方式",
    ...
    if (operation === "generateImage") {
      for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
        try {
          ...
          const usePreLlm = this.getNodeParameter(
            "usePreLlm",
            itemIndex,
          ) as boolean;
          const seed = this.getNodeParameter("seed", itemIndex) as number;
          const sizeMode = this.getNodeParameter(
            "sizeMode",
            itemIndex,
          ) as string;
          const returnUrl = this.getNodeParameter(
            "returnUrl",
            itemIndex,
          ) as boolean;
          const maxPollingTime = this.getNodeParameter(
            "maxPollingTime",
            itemIndex,
          ) as number;
          ...
          if (sizeMode === "preset") {
            const presetSize = this.getNodeParameter(
              "presetSize",
              itemIndex,
            ) as string;
          ...
          // 处理水印配置
          const logoConfig = this.getNodeParameter(
            "logoConfig",
            itemIndex,
            {},
          ) as any;
          ...
          logger(
            `图像尺寸: ${width}×${height}, 种子: ${seed}, 文本扩写: ${usePreLlm}`,
          );
          ...
          if (result.status === "done") {
            ...
            if (result.binary_data_base64 && result.binary_data_base64.length > 0) {
              ...
              logger(`✓ 成功生成 ${result.binary_data_base64.length} 张图片 (base64)`);
            }

            // 处理图片链接 (独立于 base64)
            if (returnUrl && result.image_urls && result.image_urls.length > 0) {
              outputData.image_urls = result.image_urls;
              outputData.image_count = result.image_urls.length;
              logger(`✓ 图片链接已返回 (24小时有效, 共 ${result.image_urls.length} 张)`);
            }

            // 如果两者都没有，抛出错误
            if (!outputData.binary_data_base64 && !outputData.image_urls) {
              throw new NodeOperationError(
                this.getNode(),
                "任务完成但未返回图片数据（base64 和 URL 均为空）",
                { itemIndex },
              );
            }
          }
          ...
          console.error(
            `[即梦 AI] ✗ 第 ${itemIndex + 1} 个任务失败: ${errorMessage}`,
          );
          ...
          throw new NodeOperationError(
            ...
            `即梦 AI 生成失败: ${errorMessage}`,
            {
              itemIndex,
              description:
                "请检查: 1) API 凭证是否正确, 2) 网络连接是否正常, 3) 参数配置是否合理",
            },
          );
        }
      }
